...
Все библиотеки написаны под Linux. Все кроме iniobect могут быть доработаны для работы под Windows.
...
Таймеры создаются вызовом new cTimer(интервал_в_мс, коллбэк_функция, регулярный, параметр) и удаляется с помощью delete экземпляр_таймера.
...
Примеры conf_ex, 4in1_ex требуют наличия glib-2.0 на машине, как самой библиотеки, так и заголовочных файлов. В Makefile примеров прописаны пути
так, как они выглядят на моей машине. Могут отличаться в зависимости от платформы.
