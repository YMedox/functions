...
int main() {
    ...
    while(bCont) {                                              //Незамысловатый основной цикл программы. Остановлен будет таймером с параметром
      ...
      unsigned long sleepage = timer_getSleepage(500);          //После проверки и "перезарядки" всех таймеров программа может поспать, чтобы не грузить процессор. Параметр - время простоя, если таймеров ни одного нет
      cout<<endl;
